...
where <log mountpoint> is the aforementioned mount point.)
  ...
  if virtualization.system.eql?('docker')
    ...
    if log_dir_exists
      ...
      describe("Current audit log file size is less than the specified standard of #{standard_audit_log_size}") do
        subject { log_file_size.to_i }
        it { should be <= standard_audit_log_size }
      end
      describe("Available storage for audit log should be more than the defined standard of #{standard_audit_log_size}") do
